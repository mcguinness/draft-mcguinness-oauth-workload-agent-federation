...
resources for the two independently authorized relationships.
...
This document provides a SCIM management interface for establishing and
maintaining the first three of those relationships at the IdP.

~~~
 Agent Platform                         Enterprise IdP
 +-----------------+                    +----------------------+
 | SCIM connector  | ---- SCIM -------> | Agent Principal      |
 |                 |                    | Identity Binding     |
 +-----------------+                    | Client Association   |
                                        +----------+-----------+
                                                   |
...
* Establishing trust in a new credential authority or registering an
  OAuth client. Both are prerequisites, not effects of a SCIM write.
...
* The IdP OAuth client registrations, agents, and flows the connector
  may reference when administering Client Associations.
...
The SCIM service exposes the following resource types. Schema URIs are
proposed registrations under {{iana}}.
...
All resources use SCIM `schemas`, `id`, `externalId`, and `meta`.
Additional rules for this profile are:
...
An `AgentIdentityBinding` names one resolution identity and one Agent.
The credential class determines the semantics of the identity values.
Its schema URI is:
...
| `sourceAuthority` | string | Approved authority qualified under {{classes}} |
| `sourceSubject` | string | Exact client or workload identity |
...
The reference target type of `agent` is Agent.
...
rules from {{FEDERATION}}.

| Credential class | Source authority | Source subject |
|---|---|---|
| `dedicated-client` | Trusted assertion issuer within the IdP client-registration context | Authenticated dedicated-client `client_id` and assertion `sub` |
...
The Service Provider MUST reject duplicate bindings with the same
credential class, authority, subject, and selector set in the same
Governance Tenant, regardless of enabled state. The selector set is
compared by decoded JSON Pointer paths and exact expected values,
...
Changing the source identity or destination Agent requires a new binding;
it is not an in-place credential rotation. Rotation of an authority's
...
| `clientId` | string | Exact OAuth client identifier at this IdP, not the RAS |
...
reference target type of `binding` is AgentIdentityBinding.
...
dedicated-client resolution, `clientId` MUST equal the binding's
`sourceSubject` and refer to the same IdP client registration.
...
DELETE removes the management resource and prevents its use for new
...
Deleting an Agent makes its bindings and associations unusable. Deleting
a binding makes its associations unusable. The Service Provider MUST
retain those dependent resources as non-effective references until
explicitly deleted; recreating the target with a new `id` cannot revive
them. Direct and filtered reads allow authorized connectors to find and
remove these references. A missing target MUST NOT prevent an authorized
...
|---|---|
| All three types | `id`, `externalId` |
| Agent | `active`, extension `issuer` and `subject` |
| AgentIdentityBinding | `agent.value`, `enabled` |
| AgentClientAssociation | `binding.value`, `clientId`, `flow`, `enabled` |

Collection reads MUST return only resources within the caller's read
permission. For a platform connector, an `externalId` lookup is scoped
...
| Unsupported class or flow, ambiguous authority configuration, malformed selector, or missing/invalid reference | `400`, `invalidValue` |
...
* An enabled Client Association for that binding, authenticated OAuth
  client, and flow.
...
Agent, binding, and association enablement have distinct effects. An
owner, group, or display-name update MUST NOT implicitly enable any of
...
* Purpose: Manage an approved resolution identity's binding to an Agent
  Principal.
* Single-value Attributes: `agent`, `credentialClass`, `sourceAuthority`,
...
* Purpose: Manage an OAuth client's permission to use an Identity
  Binding in a flow.
* Single-value Attributes: `binding`, `clientId`, `flow`, `enabled`,
...
workload subjects, and IdP OAuth client `platform-sso`. The token value
is a placeholder. Content-Length and repeated authorization headers are
...
`id: c3`, ETag `W/"c1"`, and its Location. This permission does not
