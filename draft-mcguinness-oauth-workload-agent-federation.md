...
This document defines how externally authenticated workloads become
Governed Agent principals at an identity provider and how those
...
grants, and states the requirements applicable to the Workload
Authorization Grant (WAG), whose wire profile remains pending upstream
changes.
...
Agent Federation defines how externally authenticated workloads become
Governed Agent principals and how those principals participate in OAuth
authorization. For agent-platform vendors and IdP implementers it is a
common integration contract: which workload evidence to provide, how to
bind it to a Governed Agent and OAuth client, and how to request
downstream authorization. It specifies the protocol behavior and trust
decisions needed for that integration. Platform-specific credential
acquisition and administrative interfaces remain deployment choices.
...
Its core is two approved relationships: an Identity Binding resolves
external evidence to a Governed Agent, and a Client Association
determines which OAuth client can use that binding in a given flow. The
grant preserves the Governed Agent as subject or actor.
...
* **IdP:** Resolve the evidence to a Governed Agent, the principal it
...
: A non-human principal within the IdP's governance domain, with a
  stable identifier, lifecycle state, and authorized external identity
  bindings.
...
* **WAG:** {{wag-flow}} states the federation requirements applicable to
  WAG. This revision claims no WAG protocol conformance; see
  {{wag-gaps}}.
...
enrollment or key-replacement protocols. {{upstream-gaps}} records
these dependencies; {{operational-guidance}} gives deployment guidance.

## Walkthrough: Shared Platform Client {#identity-example}
...
* Credential-authority trust policy, approved keys or key-source
  ...
  class. Attester trust follows {{agent-evidence}}.
* Exact external identity selectors and their Governance Tenant and
  Governed Agent associations.
...
approved Identity Binding or Client Association.

The IdP MUST authorize creation and changes of Identity Bindings and
Client Associations, including imports from platform registries.
Administrative mechanisms are outside this profile
...
an identity model or establish an Identity Binding.

Two key relationships need distinct names. An issuer-bound presenter
key is a key the credential issuer has attested belongs to the
workload, such as the `cnf` option in {{platform-jwt-format}} or an
attestation's confirmation key. A request proof key is a key the
presenter proves in the request without issuer attestation, as with
DPoP accompanying bearer evidence. Only the former establishes that the
...
identifier. Multiple approved Identity Bindings MAY identify the same
Governed Agent.
...
by itself create a new authorization principal.

# Workload Evidence {#evidence}

Agent Federation requires evidence that authenticates an
issuer-qualified external workload identity to the IdP. The platform
...
authenticates an issuer-qualified external workload identity to the IdP
...
and attestation credentials can satisfy the same requirement through
the inputs in {{optional-inputs}}. The IdP and platforms implementing
the common path MUST support it.
...
The issuer and subject pair MUST identify one stable platform workload, unique across the issuer's tenants and not reassigned to a
...
Identity Binding supplies the Governed Agent and Governance Tenant.
...
The platform MUST issue credentials with a finite lifetime. The IdP MUST
validate expiration and reject future issuance times beyond its
configured clock skew. It MUST configure and enforce finite maximum age
and issuance-to-expiration limits. These limits are deployment
parameters that account for platform issuance and caching.
...
Agent identifier or OAuth `client_id`. A failed fixed-profile check MUST
NOT fall back to imported JWT processing.

Recipients MUST NOT accept this credential as an OAuth client assertion,
authorization grant, or API access token.
...
The OPTIONAL imported platform JWT input accommodates existing platform
...
For an imported workload, the Identity Binding MUST specify an exact
...
MUST be present, have the configured type, and match exactly.
...
If `cnf` is present, the IdP MUST enforce its defined proof mechanism
and reject an unsupported binding; such a credential MUST NOT receive
bearer treatment.
...
The IdP MUST use configured attester-to-client and key associations. For
an agent with its own client, it MUST identify the attester
unambiguously from the trusted verification key and resolve that
attester and the validated client `sub` through an approved Identity
...
replay, and key-use requirements. The IdP MUST apply them to every
presentation and check the current Identity Binding and Client
Association. DPoP MUST NOT substitute for another required credential
proof.

For bearer JWT-SVID and unbound platform JWT inputs, co-presentation
with DPoP does not establish issuer endorsement of the proof key. A
deployment requiring that assurance MUST use evidence that
cryptographically binds the key, such as the `cnf` option in
{{platform-jwt-format}}. Bearer-evidence theft remains a threat even
when the output is sender-constrained ({{security}}). Shared workload
identities do not distinguish replicas ({{instance-identification}}).
...
| Client Attestation, agent has its own client | Attester selected under {{agent-evidence}}; client `sub` | Client-to-agent mapping is explicit |
...
Governed Agent through an enabled binding. It MUST reject missing,
...
A client identifier, including a {{CIMD}} URL, identifies the OAuth
client, not the agent. Even where the credential specification permits a
client-identifier association or prefix match, the Identity Binding MUST
resolve the exact workload identity selected as actor evidence to one
Governed Agent.
...
MUST NOT satisfy this check. The IdP MUST NOT substitute the client's
identity for the resolved actor.

## Subject Resolution and Linking {#subject-resolution}

For ID-JAG, subject resolution identifies the user; linking associates
that external identity with a local account. WAG subject resolution
follows {{wag-flow}}.
...
The Governed Agent identity is the IdP issuer and Governed Agent identifier:
...
links MUST remain distinct. A changed Identity Binding or local agent
...
using the exact pair (IdP issuer, Governed Agent identifier). This is
...
acting relationship, Governance and Target Tenants, RAS, resource, and
...
authority MUST NOT exceed the user's authority and the agent's
authorized delegation under {{delegation-authorization}}. This
delegation is a gate on acting for the user, not a requirement for
independent agent ownership of each data permission
({{actor-authorization}}).

The IdP MAY grant an authorized non-empty scope subset. It MUST return
`invalid_scope` if none can be granted, or if policy forbids a reduced
grant and the request exceeds the authorized scope. Any reduction MUST
be reflected in the grant and response.

The IdP MUST NOT issue a grant by dropping a required actor or binding,
substituting an external identifier for the Governed Agent identity, or
...
Governed Agent actor in authorization or attribution.
...
* A Client Association permitting its authenticated OAuth client to use
  the selected Identity Binding in this flow with the selected actor
  credential class.
...
if `act` or `cnf` is omitted from a presented ID-JAG.

Each authorization server MUST establish authoritative client metadata
through registration or, when supported, {{CIMD}}. The client and
servers MUST support `private_key_jwt` under {{RFC7523, Section 2.2}}.
Other configured methods MAY be used. Client identifiers and keys MAY
...
credential still selects exactly one approved Identity Binding. No
per-agent OAuth client or per-replica registration is required.

For `private_key_jwt`, the client MUST use the receiving token endpoint
URL as the assertion audience; this selects one RFC 7523 audience
option. Other methods use their own audience rules.
...
* **ID Token:** Validate under ID-JAG; the audience MUST identify the
...
under {{delegation-authorization}}. A refresh token replaces the ID
Token input; the output remains an ID-JAG.
...
associations. The IdP MUST reject ambiguous classification.
...
* **Imported platform JWT:** Apply {{imported-jwt-input}}, including
  audience and any `cnf` binding.
...
{{ID-JAG, Section 9.8.1.2}} to that grant type; the `jwt-bearer` name
does not permit ignoring the assertion's `cnf`
({{bound-grant-coordination}}).
...
4. **Local authorization:** Resolve the user under
   {{ras-subject-resolution}} and the Governed Agent actor under
...
Refresh-token issuance follows {{ras-refresh}}.

An unexpired ID-JAG remains reusable under ID-JAG. Each redemption
requires fresh proof validation and current RAS policy.

### Access-Token Protection {#access-token-protection}

The RAS MUST issue a sender-constrained access token unless the resource
is explicitly configured to permit bearer tokens. The client and RAS
MUST select the permitted protection through trusted client and resource
...
support; grant proof validation remains mandatory. The RAS MUST NOT copy
the grant's `cnf` into an access token whose binding will not be
enforced. Clients and APIs MUST NOT treat a constrained token as an
unconstrained bearer token or bypass an unrecognized confirmation
method.

### RAS Refresh Tokens {#ras-refresh}

RAS refresh preserves an existing authorization at the same RAS;
Identity Continuation Assertion {{ICA}} establishes identity continuity
when authorization crosses into another authorization domain
({{continuing-access}}).
...
RAS refresh under {{ras-refresh}} preserves an existing authorization at
the same RAS. Identity Continuation Assertion {{ICA}} establishes
identity continuity when authorization crosses into another
authorization domain; that composition is separate work
...
issuance and consumption; it is not a WAG wire profile. An
interoperable IdP-issued, sender-constrained WAG remains pending
{{wag-gaps}}, and this revision claims no WAG conformance.
...
Any additional target-local subject identifier for WAG needs its
authority, tenant scope, and consistency with `sub` specified with WAG
({{wag-gaps}}).
...
Actor Profile metadata MAY advertise independently supported paths.
Servers MUST NOT advertise its full Token Exchange algorithm solely for
implementing this profile. If published, `actor_profile_token_exchange`
MUST describe those paths and agree with shared ID-JAG capabilities.
...
Grant redemption requires the issuance DPoP key. Distributing issuance
and redemption across components requires shared control of that key,
which widens its exposure; see {{flow-configuration}}.
...
identifiers out of the ID-JAG. The RAS preserves the Governed Agent actor
...
| Self-acting subject resolution and agent linking | This profile, {{wag-flow}} |
...
* **Actor mapping:** How ICA's authenticated-client identity corresponds
  to the Governed Agent resolved at the root. A shared client alone
...
* **Binding continuity:** Which root Identity Binding and Client
...
that principal authority. {{wag-flow}} defines this
...
qualify for either source. Requiring the same Governed Agent actor does not
...
Governed Agent identity under {{actor-construction}}. Repeating that mapping
...
authentication alone does not select the Governed Agent actor.
...
this profile's independent Identity Binding, Client Association, and delegation checks.
